...
public class MemberController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description =
                    "유효하지 않은 이메일 형식(MEMBER_01), " +
                    "유효하지 않은 날짜 형식(MEMBER_03), " +
                    "이미 존재하는 이메일(MEMBER_04)")
    })
    @Operation(summary = "이메일 가입", description = "이메일로 회원 가입하는 API입니다.")
    @PostMapping("/signup/email")
    public ResponseEntity<Profile> signup(@Valid @RequestBody MemberSignupByEmailRequest request) {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201",  description = "회원 가입 성공"),
            @ApiResponse(responseCode = "400", description =
                            "유효하지 않은 전화 번호 형식(MEMBER_02), " +
                            "유효하지 않은 날짜 형식(MEMBER_03), " +
                            "이미 존재하는 전화번호(MEMBER_05)")
    })
